...
const submitUserTransactionDetails = async (event) => {
  ...
  try {
    await fetch(`/api/transactions/${props.currentTransaction.id}`, {
      method: 'PATCH',
      headers: {
    ...
    >
      <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
        <div>
          ...
          >
            <option :selected="props.currentTransaction?.type === 'CREDIT'">
              CREDIT
            </option>
        ...
        <div>
          ...
          >
            ...
            <option
              :selected="props.currentTransaction?.status === 'FULFILLED'"
            >
              FULFILLED
            </option>
            <option
              :selected="props.currentTransaction?.status === 'CANCELLED'"
            >
              CANCELLED
            </option>
